...
from homeassistant.data_entry_flow import TranslationFieldMapping, section
...
class SurePetCareConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_pet_select_config(
        ...
        data_schema, translation_field_mappings = self._get_pet_select_config_schema(
            user_input
        )
    ...
    def _build_pet_select_config_entry(
        ...
        config_entry_data[CONF_PET_SELECT_OPTIONS] = list(selected_zones)

        return config_entry_data

    def _get_pet_select_config_schema(
        self, default_input: dict[str, Any] | None
    ) -> tuple[vol.Schema, dict[str, TranslationFieldMapping]]:
        ...
        for flap in self._flaps:
            flap_id = str(flap.id)
            translation_field_mappings[flap_id] = {
                "translation_key": "flap",
                "description_placeholders": {"flap_name": flap.name},
            }
            flap_default_input = default_input.get(flap_id, {})
            data_schema[vol.Required(flap_id)] = section(
        ...
        return vol.Schema(data_schema), translation_field_mappings
    ...
    def _validate_pet_select_config_user_input(
        ...
        for flap in self._flaps:
            flap_device_id = str(flap.id)
            value = user_input[flap_device_id]
            if value["entry"].strip() == "" or value["exit"].strip() == "":
                errors[flap_device_id] = "no_zones_selected"
                return errors
